class Admin::BlankFormsController < ApplicationController
  ...
  def create
    @departments = Department.all
    section = BlankForm.where(section: params[:blank_form][:section_name]).first
    if params[:blank_form][:section_name].present? && section.present?
      params[:blank_form][:section] = section.section
  ...
  def get_sections
    @sections = BlankForm.where(department_id: params[:department_id]).pluck(:section).uniq
  end
  ...
  def blank_form_params
    params.require(:blank_form).permit(:department_id, :eng_name, :hindi_name, :section, :section_hindi, :category, :content)
  end
